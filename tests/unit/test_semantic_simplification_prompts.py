...
class TestSemanticSimplificationPrompts(unittest.TestCase):
    def test_deep_is_free_form_sole_judgment_with_three_review_passes(self):
        ...
        for phrase in (
            ...
            "Context synthesis",
            "Falsification",
            "Begin the memo in this exact semantic order",
            "`PR objective`",
            ...
            "A `test-gap` may carry a blocking decision only when",
            "`question` and `note` items always remain nonblocking",
    ...
    def test_final_has_one_fixed_presentation_only_transport(self):
        ...
        for boundary in (
            ...
            "Preserve Deep's opening `PR objective`, `Objective closure`, and `Merge posture` commitments",
            ...
            "For every blocking decision, put the single finding that most affects the merge posture first",
            "For `verification_needed`, put the single missing fact that most affects the merge posture first",
            "reuses that first surviving deciding item and its own owner action for the first screen",
            ...
            "A `test-gap` may be that carrier only when Deep explicitly made it a pre-merge blocker",
            "`question` and `note` never carry a blocking decision",
